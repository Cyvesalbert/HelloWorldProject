...
public class TicTacToeServer extends JFrame{
	...
	private Lock gameLock; // to lock game for syncronization
	private Condition otherPlayerConnected; // to wait for other player
	private Condition otherPlayerTurn; // to wait for other player's turn
	
	// set up tic tac toe server and GUI that display messages
	public TicTacToeServer() {
		super("Tic-tac-Toe Server"); // set titleof window
		...
		private void displayMessage(final String messageToDisplay) {
			...
			SwingUtilities.invokeLater(
					new Runnable() {
						public void run() { // updates output area
		...
		public boolean validateAndMove(int location, int player) {
			...
			if(!isOccupied(location)) {
				...
				try {
					...
					gameLock.unlock(); // unlockgame after signalling
				}
		...
		// private inner class Player namages each Player as a runnable
		private class Player implements Runnable{
			private Socket connection; // connection to client
			private Scanner input; // input to client
			private Formatter output; // output to client
			private int playerNumber; // tracks which player this is
			private String mark; // mark for this player
			private boolean suspended = true; // wheter thread is suspended
			
			// set up player thread
			public Player(Socket socket, int number) {
				playerNumber = number; // store this player's number
				mark = MARKS[playerNumber]; // specify players mark
			...
			public void run() {
				...
				try {
					...
					if(playerNumber == PLAYER_X) {
						...
						// send message thet other player connected
						output.format("Player O connected, your move\n");
						output.flush();
					}else {
